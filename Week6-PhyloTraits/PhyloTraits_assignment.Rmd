...
getwd() 
setwd("/Users/kbeidler/GitHub/QB2017_Beidler/Week6-PhyloTraits")
...
grid(ncol(window.full), nrow(window.full), col = "lightgrey")
```
...
# You can perform model selection with either an ANOVA test or by AIC value anova(fitJC, fitGTR)
AIC(fitJC)
AIC(fitGTR)
...
ml.bootstrap = read.tree("./data/ml_tree/RAxML_bipartitions.T1") par(mar = c(1, 1, 2, 1) + 0.1)

plot.phylo(ml.bootstrap, type = "phylogram", direction = "right",
    show.tip.label = TRUE, use.edge.length = FALSE, cex = 0.6,
label.offset = 1, main = "Maximum Likelihood with Support Values") add.scale.bar(cex = 0.7)
