...
getwd() 
<<<<<<< HEAD
setwd("/Users/beidler/downloads")

# Loading Required Packages
package.list = c('vegan', 'ade4', 'viridis', 'gplots','indicspecies') 
=======
setwd("/Users/kbeidler/GitHub/QB2017_Beidler/Week3-Beta")
...
<<<<<<< HEAD
> ***Answer 2c***: While richness tended to be higher downstream, the composition of the community likely differed between upstream and downstream regions of the stream (many of the downstream sites did not have any Brown Trout present). Therefore there are limitations associated with richness when examining patterns of biodiversity across sites, because richness may be the same among sites but species composition may differ.  
=======
...
> ***Answer 3a***: $\beta_{w} = 2.16. This index quantifies how many more times more diverse the regional species pool ($\gamma$ = 27) is than the average richness at each site within the region ($\alpha$ = 12.5 in this case). For the River Doubs ($\gamma$) fish diversity is around double ($\alpha$) fish diversity. So ($\gamma$) fish diversity is due to high richness and low turnover.
...
=======
> ***Answer 3c***: If you defined beta additively (i.e., $\beta = \gamma - \alpha$) then beta would be 14.5 and the interpretation would change from 3a. as beta and alpha would be closer in value. 
>>>>>>> origin/master
...
> ***Answer 7b***:The following species can be used as indicator species for river quality: 
+ *Phoxinus phoxinus*
...
# Habitat preferences of each species. In this case, we can calculate the phi coefficient of association. Values of the phi coefficient range from −1 (suggesting strong avoidance of a group) to 0 (suggesting no preference) to +1 (suggesting strong preference for a group).
